...
void mult_matrix_by_vector(const struct mat *A, const double *v, double *result, bHat *bHat_g, int bool, int bool_1){
	...
	for (i = 0 ; i < size; i++){
		...
		for (j = 0; j < size; j++) {
			...
			if (i == j && bool_1 == 1){
				...
				while(node != NULL && node -> index != cur){
					if (node -> index > cur)
						break;
					node = node -> next;
				}

				if (node != NULL && node -> index == cur){
					dotPro += (1 - (elem2))* (*(v + j));
